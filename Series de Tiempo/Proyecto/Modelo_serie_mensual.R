...
XAG<-ts(XAG)
XAG<- ts(XAG,frequency=12,start=c(2011,2))
length(XAG)
plot.ts(XAG, col='blue')

#lambda para estabilizar la varianza
#serie transformada XAG^-1
XAG_t<-1/XAG
length(XAG_t)
plot.ts(XAG_t, col='blue')
XAG_d1<-diff(XAG_t)#primera diferencia a la serie transformada
length(XAG_d1)
plot.ts(XAG_d1,col='blue')
XAG_d2<-diff(XAG_d1)  #segunda diferencia a la serie transformada
length(XAG_d2)
plot.ts(XAG_d2,col='blue')

#desviaciones estandar de las diferencias
sd(XAG_t)
sd(XAG_d1)   #minima, por lo tanto trabajaremos con 1 diferencia
sd(XAG_d2)

#funciones de autocorrelacion
acf(XAG_t)# no hay un decaimiento rapido hacia cero
plot(acf(XAG_d1, type = "correlation",lag.max = 24),xlab = "k", main = "",ylim = c(-0.3,0.3))
#decaimiento rapido hacia cero, y algunos valores significativamente diferentes de cero
plot(acf(XAG_d2, type = "correlation",lag.max = 24),xlab = "k", main = "",ylim = c(-0.3,0.3))
#primera autocrrelacion muy grande
plot(pacf(XAG_d1,lag.max = 24),xlab = "k", main = "",ylim = c(-0.3,0.3))
pacf(XAG_d2)

#por los resultados de la FAC y de las difencias, trabajaremos con 1 diferencia
#como no hay autocorrelaciones significativamente distintas de cero 
#no tenemos evidencias para proponer MA o AR entonces la primera propuesta seria ARIMA(0,1,0)
###############

#modelo que mejor se ajuste a los datos propuesto por R
auto.arima(XAG_d1)
auto.arima(XAG_t)
#ARIMA(0,1,0) para la serie transformada
modelo1<-arima(XAG_t,c(0,1,0))  #es mejor usar este para pronósticos
residuales<-as.vector(modelo1$residuals)
plot(residuales) #no hay residuales significativamente distintos de cero
